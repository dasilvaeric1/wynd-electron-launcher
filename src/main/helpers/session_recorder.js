...
async function inject(posWc) {
  ...
  try { await posWc.executeJavaScript(INJECT(src), true); return true; }
  catch (e) { log.warn(`[REC] inject KO: ${e.message}`); return false; }
}
...
async function drain() {
  ...
  try {
    const batch = await active.posWc.executeJavaScript(
      '(()=>{const e=window.__rrwebEvents||[];window.__rrwebEvents=[];return e})()', true);
    if (Array.isArray(batch) && batch.length) active.events.push(...batch);
  } catch (e) { log.debug(`[REC] drain err: ${e.message}`); }
  if (active.events.length > MAX_EVENTS || Date.now() - active.startedAt >= MAX_DURATION_MS) {
    log.info('[REC] auto-stop (cap atteint)'); stop().catch(() => {});
  }
}

function start({ posWc, cfg, posUrl, clientHint, onStatus }) {
  if (active) return;
  active = { posWc, cfg, posUrl, clientHint, startedAt: Date.now(), events: [], net: [], timer: null, onStatus };
  inject(posWc).then((ok) => { if (ok && onStatus) onStatus({ state: 'recording' }); });
  active.timer = setInterval(drain, DRAIN_MS);
}

function addNet(msg) { if (active) active.net.push({ ...msg, ts: Date.now() }); }

function injectIfRecording(posWc) { if (active) inject(posWc); }
...
async function stop() {
  ...
  // final drain
  try {
    const batch = await a.posWc?.executeJavaScript?.('(()=>{const e=window.__rrwebEvents||[];window.__rrwebEvents=[];if(window.__elRec)window.__elRec();window.__elRec=null;return e})()', true);
    if (Array.isArray(batch)) a.events.push(...batch);
  } catch (_) {}
  const startedAt = new Date(a.startedAt).toISOString();
  const stoppedAt = new Date().toISOString();
  const meta = { clientId: a.clientHint, caisseSerial: a.cfg.serial, startedAt, stoppedAt,
    launcherVersion: require('../../../package.json').version, posUrl: a.posUrl };
  const zip = new JSZip();
  ...
  try {
    const pres = await axios.post(`${a.cfg.baseUrl}/api/traces/presign`,
      { caisseSerial: a.cfg.serial, startedAt },
      { headers: { 'x-api-key': a.cfg.apiKey }, timeout: 15000 });
    const { traceId, uploadUrl } = pres.data;
    await axios.put(uploadUrl, buf, { headers: { 'content-type': 'application/zip' }, timeout: 120000, maxBodyLength: Infinity, maxContentLength: Infinity });
    await axios.post(`${a.cfg.baseUrl}/api/traces/${traceId}/complete`,
      { sizeBytes: buf.length, stoppedAt, durationMs: a.startedAt ? (Date.now() - a.startedAt) : 0, meta },
      { headers: { 'x-api-key': a.cfg.apiKey }, timeout: 15000 });
